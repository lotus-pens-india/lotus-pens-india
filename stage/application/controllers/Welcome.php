...
class Welcome extends CI_Controller
{
	...
	public function product($product_id)
	{
		...
		if ($products != false && count($products) > 0) {
			...
			$relatedProducts = $this->GlobalModal->executeQuery("SELECT VP.*,PP.price as unit_price FROM vegshopy_product VP
			inner join lp_product_price PP on PP.product_id=VP.product_id
			where PP.currency='" . $currency . "' and VP.status=1 and VP.product_id!=" . $product_id . " and VP.category_id=" . $products[0]['category_id'] . " order by VP.position asc, VP.product_id desc limit 8");
			...
			'price' => $price, 'withClipAmt' => $withClipAmt, 'reviews' => $reviews, 'relatedProducts' => $relatedProducts,'metaData'=>$metaData));
			$this->load->view('welcome_message', $data);
		} else {
